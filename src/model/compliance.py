...
def compute_gap_metrics(results):
    ...
    for r in valid:
        ...
        if comp.get("intended_optout"):  intended_count  += 1
        if comp.get("effective_optout"): effective_count += 1
        if comp.get("gap_identified"):   fallacy_count   += 1

        country = r.get("country", "UNKNOWN")
        by_country.setdefault(country, {"total": 0, "compliant": 0, "gap": 0})
        by_country[country]["total"] += 1
        if status == "COMPLIANT":      by_country[country]["compliant"] += 1
        if status in ("NOMINAL", "NON_COMPLIANT"): by_country[country]["gap"] += 1

        tier = r.get("strategy_tier", "UNKNOWN")
        by_tier.setdefault(tier, {"total": 0, "compliant": 0, "gap": 0})
        by_tier[tier]["total"] += 1
        if status == "COMPLIANT":      by_tier[tier]["compliant"] += 1
        if status in ("NOMINAL", "NON_COMPLIANT"): by_tier[tier]["gap"] += 1
